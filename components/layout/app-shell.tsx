...
export default function AppShell({ user, children }: { user: { fullName: string; email: string; role: Role }; children: ReactNode }) {
  ...
  return <div className="insight-shell">
    ...
    <aside className={`insight-sidebar ${open ? "open" : ""} ${collapsed ? "collapsed" : ""}`}>
      <div className="insight-brand-row"><Link className="insight-brand" href={isViewer ? "/dashboards" : "/overview"} onClick={() => setOpen(false)}><IfsLogo variant="light" markOnly={collapsed}/></Link><button type="button" className="shell-collapse-toggle" aria-label={collapsed ? "Expand navigation" : "Collapse navigation"} aria-expanded={!collapsed} title={collapsed ? "Expand navigation" : "Collapse navigation"} onClick={() => setCollapsed((current) => !current)}><Icon name={collapsed ? "chevronRight" : "chevronLeft"}/></button></div>

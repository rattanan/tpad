...
export default function BusinessFieldPicker({ label, value, fields, onChange, helpText }: {
  ...
  useEffect(() => {
    if (!value || requestedSamples.current.has(value)) return;
    requestedSamples.current.add(value);
    let active = true;
    setSamples((current) => ({ ...current, [value]: { status: "loading", values: [], masked: false } }));
    void fetch(`/api/business-fields/${value}/sample`, { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" })
      .then(async (response) => {
  ...
  return <div className="business-field-control" ref={rootRef} onKeyDown={(event) => { if (event.key === "Escape") setOpen(false); }}>
    ...
    {selected && <div className="business-field-sample" aria-live="polite">
      ...
      {sample?.status === "loading" && <small>Loading sample values…</small>}
      ...
      {sample?.status === "error" && <small>Sample data is unavailable for this source.</small>}
    </div>}

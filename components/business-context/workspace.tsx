...
function ObjectsTab({ data, editable }: { data: Workspace; editable: boolean }) {
  ...
  async function generateDrafts() {
    ...
    try {
      const response = await fetch(`/api/business-context-models/${data.model.id}/generate-business-objects`, { method: "POST" });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Unable to generate Draft Business Objects");
      setNotice(`${body.createdCount} Draft Business Object${body.createdCount === 1 ? "" : "s"} created${body.skippedNonMeasureCount ? ` · ${body.skippedNonMeasureCount} master-only/no-measure object${body.skippedNonMeasureCount === 1 ? "" : "s"} skipped` : ""}${body.skippedEmptyCount ? ` · ${body.skippedEmptyCount} object${body.skippedEmptyCount === 1 ? "" : "s"} without Measure data skipped` : ""}`);
      router.refresh();
  ...
  return <>
    ...
    <div className="bc-object-grid">{data.objects.map((object) => <article className={editable ? "has-object-admin" : ""} key={object.id}><button type="button" className="business-object-preview" aria-label={`View sample data for ${object.businessName}`} title="View top 10 sample rows" disabled={previewLoading} onClick={() => void showSample(object)}><TableIcon /></button>{editable && <button type="button" className="business-object-delete" aria-label={`Remove ${object.businessName}`} title="Remove Business Object" disabled={Boolean(removing) || generating} onClick={() => void removeObject(object)}>{removing === object.id ? <span className="button-spinner" /> : <TrashIcon />}</button>}<div><span>{object.objectType.replaceAll("_", " ")}</span>{badge(object.approvalStatus)}</div><h3>{object.businessName}</h3><p>{object.description || "Business description pending review."}</p><dl><div><dt>Physical source</dt><dd>{object.databaseSchema}.{object.technicalName}</dd></div><div><dt>Record grain</dt><dd className={!object.recordGrain ? "missing" : ""}>{object.recordGrain || "Missing — required before approval"}</dd></div></dl><small>{fieldsCount(data.fields, object.id)} mapped fields</small></article>)}</div>
...
function KpisTab({data,editable}:{data:Workspace;editable:boolean}){
  ...
  const router=useRouter();const [busy,setBusy]=useState('');const [error,setError]=useState('');const [notice,setNotice]=useState('');const [analysis,setAnalysis]=useState<KpiAnalysis|null>(null);
  async function generate(){setBusy('generate');setError('');setNotice('');setAnalysis(null);const response=await fetch(`/api/business-context-models/${data.model.id}/generate-kpis`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});const body=await response.json();setBusy('');if(!response.ok){setError(body.error??'Unable to generate KPI drafts');return;}setAnalysis(body.analysis??null);setNotice(`${body.createdCount} KPI draft${body.createdCount===1?'':'s'} created · ${body.profiledFieldCount} fields profiled · ${body.excludedFieldCount} fields excluded`);router.refresh();}

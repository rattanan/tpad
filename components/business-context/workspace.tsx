...
export default function BusinessContextWorkspace({dataJson,role,focusApproval=false,returnTo}:{dataJson:string;role:Role;focusApproval?:boolean;returnTo?:string}){const data=JSON.parse(dataJson) as Workspace;const router=useRouter();const [tab,setTab]=useState<(typeof tabs)[number]>("Overview");const [busy,setBusy]=useState("");const [notice,setNotice]=useState("");const [error,setError]=useState("");const [validation,setValidation]=useState<{outcome:string;issues:Array<{code:string;severity:string;message:string}>}|null>(null);const editable=(role==="ADMIN"||role==="DATA_SOURCE_CREATOR")&&['DRAFT','CHANGES_REQUESTED'].includes(data.model.status);async function action(path:string,label:string,body:object={}){setBusy(label);setError("");setNotice("");const response=await fetch(path,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});const result=await response.json();setBusy("");if(!response.ok){setError(result.error??"Action failed");return null;}setNotice(`${label} completed`);router.refresh();return result;}const fieldsByObject=useMemo(()=>new Map(data.objects.map((object)=>[object.id,data.fields.filter((field)=>field.businessObjectId===object.id)])),[data.objects,data.fields]);return <main className="bc-workspace"><header className="bc-model-head"><div><Link href="/business-context-models">← Business Context Models</Link><div className="bc-title-line"><h1>{data.model.name}</h1>{badge(data.model.status)}</div><p>{data.model.description||`${data.model.schemaName} governed business metadata`}</p><small>Oracle schema {data.model.schemaName} · Version {data.model.version} · Updated {new Date(data.model.updatedAt).toLocaleString()}</small></div><div className="bc-actions"><button className="secondary-button" disabled={Boolean(busy)} onClick={async()=>{const result=await action(`/api/business-context-models/${data.model.id}/validate`,"Validation");if(result)setValidation(result as typeof validation);setTab("Validation");}}>Validate</button>{editable&&['DRAFT','CHANGES_REQUESTED'].includes(data.model.status)&&<button className="primary-button" disabled={Boolean(busy)} onClick={()=>void action(`/api/business-context-models/${data.model.id}/submit-review`,"Submit review")}>Submit review</button>}{role==="ADMIN"&&data.model.status==="READY_FOR_REVIEW"&&<button className="primary-button" onClick={()=>void action(`/api/business-context-models/${data.model.id}/approve`,"Approval")}>Approve</button>}{role==="ADMIN"&&data.model.status==="APPROVED"&&<button className="primary-button" onClick={()=>void action(`/api/business-context-models/${data.model.id}/publish`,"Publish",{changeSummary:`Publish version ${data.model.version}`})}>Publish</button>}{data.model.status==="PUBLISHED"&&(role==="ADMIN"||role==="DATA_SOURCE_CREATOR")&&<button className="primary-button" onClick={()=>void action(`/api/business-context-models/${data.model.id}/create-version`,"Create version")}>Create new version</button>}</div></header>{error&&<div className="bc-alert error">{error}</div>}{notice&&<div className="bc-alert success">{notice}</div>}{focusApproval&&<ApprovalWorkflow model={data.model} objects={data.objects} fields={data.fields} role={role} busy={busy} returnTo={returnTo} action={action}/>}<nav className="bc-tabs" aria-label="Business Context sections">{tabs.map((item)=><button key={item} className={tab===item?"active":""} onClick={()=>setTab(item)}>{item}</button>)}</nav><section className="bc-tab-panel">{tab==="Overview"&&<Overview data={data}/>} {tab==="Business Objects"&&<ObjectsTab data={data} editable={editable}/>} {tab==="Fields"&&<FieldsTab modelId={data.model.id} fields={data.fields} editable={editable}/>} {tab==="Relationships"&&<RelationshipsTab data={data} fieldsByObject={fieldsByObject} editable={editable}/>} {tab==="KPI Catalogue"&&<KpisTab data={data} editable={editable} role={role}/>} {tab==="Validation"&&<ValidationTab result={validation}/>} {tab==="Versions"&&<VersionsTab data={data} role={role}/>} {tab==="Audit Log"&&<AuditTab model={data.model} role={role}/>}</section></main>}
function ApprovalWorkflow({model,objects,fields,role,busy,returnTo,action}:{model:Model;objects:ObjectRow[];fields:Field[];role:Role;busy:string;returnTo?:string;action:(path:string,label:string,body?:object)=>Promise<unknown>}){const approved=['APPROVED','PUBLISHED'].includes(model.status);const canSubmit=(role==='ADMIN'||role==='DATA_SOURCE_CREATOR')&&['DRAFT','CHANGES_REQUESTED'].includes(model.status);const canApprove=role==='ADMIN'&&model.status==='READY_FOR_REVIEW';return <section id="approval-workflow" className={`context-approval-guide ${approved?'complete':''}`}><div className="context-approval-icon" aria-hidden="true">{approved?'✓':'!'}</div><div><p className="eyebrow">SOURCE APPROVAL REQUIRED</p><h2>{approved?'Business Context source metadata is approved':'Approve the Business Context before KPI review'}</h2><p>Business Objects and Fields are approved together with the Business Context. There is no separate Approve button on each Object or Field.</p><ol><li className={model.status!=='DRAFT'?'done':''}><span>1</span><div><strong>Submit context for review</strong><small>Runs context validation and changes the model from Draft to Ready for Review.</small></div></li><li className={approved?'done':''}><span>2</span><div><strong>Admin approves Business Context</strong><small>Automatically approves {objects.length} Business Object{objects.length===1?'':'s'} and {fields.length} Business Field{fields.length===1?'':'s'}.</small></div></li><li className={approved?'done':''}><span>3</span><div><strong>Return to KPI and Validate</strong><small>The KPI can be submitted after its latest version passes validation.</small></div></li></ol></div><div className="context-approval-action">{canSubmit&&<button className="primary-button" disabled={Boolean(busy)} onClick={()=>void action(`/api/business-context-models/${model.id}/submit-review`,'Submit context for review')}>{busy==='Submit context for review'&&<span className="button-spinner"/>}Submit context for review</button>}{canApprove&&<button className="primary-button" disabled={Boolean(busy)} onClick={()=>void action(`/api/business-context-models/${model.id}/approve`,'Approve Business Context')}>{busy==='Approve Business Context'&&<span className="button-spinner"/>}Approve Business Context</button>}{model.status==='READY_FOR_REVIEW'&&role!=='ADMIN'&&<span className="bc-status ready_for_review">Waiting for Admin approval</span>}{approved&&returnTo&&<Link className="primary-button" href={returnTo}>Return to KPI and Validate</Link>}</div></section>}

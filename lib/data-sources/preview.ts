...
export async function previewColumnValues(dataSourceId:string,tableId:string,columnName:string,rowLimit=5){
  const source=await getDataSource(dataSourceId);if(!source)throw new HttpError(404,"Data source not found","NOT_FOUND");
  const table=(await db.select().from(dataSourceTables).where(and(eq(dataSourceTables.id,tableId),eq(dataSourceTables.dataSourceId,dataSourceId),eq(dataSourceTables.isIncluded,true),eq(dataSourceTables.status,"ACTIVE"))).limit(1))[0];if(!table)throw new HttpError(404,"Table is not available","NOT_FOUND");
  const column=(await db.select({columnName:dataSourceColumns.columnName,sensitivityType:dataSourceColumns.sensitivityType}).from(dataSourceColumns).where(and(eq(dataSourceColumns.tableId,tableId),eq(dataSourceColumns.columnName,columnName),eq(dataSourceColumns.isIncluded,true),eq(dataSourceColumns.status,"ACTIVE"))).limit(1))[0];if(!column)throw new HttpError(404,"Column is not available","NOT_FOUND");
  const limit=Math.min(10,Math.max(1,rowLimit));const selected=quote(column.columnName);const sourceSql=`SELECT ${selected} FROM ${quote(table.schemaName)}.${quote(table.tableName)} WHERE ${selected} IS NOT NULL`;
  const rows=await withOracleConnection(source,async(connection,outFormat)=>{try{return(await connection.execute(`${sourceSql} FETCH FIRST :rowLimit ROWS ONLY`,{rowLimit:limit},{outFormat,maxRows:limit})).rows??[];}catch(error){if(!String((error as Error).message).includes("ORA-00933"))throw error;return(await connection.execute(`SELECT * FROM (${sourceSql}) WHERE ROWNUM <= :rowLimit`,{rowLimit:limit},{outFormat,maxRows:limit})).rows??[];}}) as Array<Record<string,unknown>>;
  return{values:rows.map(row=>maskValue(row[column.columnName],column.sensitivityType as Sensitivity)),masked:column.sensitivityType!=="NONE"};
}

...
export async function runBusinessContextAnalysis(modelId: string, user: AuthenticatedUser, provider: BusinessContextAnalysisProvider = new IfsPatternAnalysisProvider()) {
  ...
  const mappedObjects = await db.select({ physicalTableId: businessObjects.physicalTableId }).from(businessObjects).where(and(eq(businessObjects.modelId, modelId), isNull(businessObjects.deletedAt))); const mappedTableIds = new Set(mappedObjects.map((item) => item.physicalTableId));
  const selected = (mappedTableIds.size ? tables.filter((table) => mappedTableIds.has(table.id)) : tables).slice(0, 100); const tableIds = new Set(selected.map((table) => table.id)); const allColumns = selected.length ? await db.select().from(dataSourceColumns).where(and(inArray(dataSourceColumns.tableId, selected.map((table) => table.id)), eq(dataSourceColumns.status, "ACTIVE"))) : []; const columns = allColumns.slice(0, Math.max(0, 10_000 - selected.length));
  ...
  try {
    ...
    const parsed = recommendationsSchema.safeParse(raw); if (!parsed.success) throw new HttpError(422, `AI analysis output is invalid or too large (${raw.length} recommendations). Reduce included metadata or map the required Business Objects first.`, "AI_RECOMMENDATION_VALIDATION_FAILED"); const recommendations = parsed.data; const completedAt = new Date();

...
export async function validateKpi(kpiId: string, user: AuthenticatedUser) {
  const kpi = (await db.select().from(kpiDefinitions).where(and(eq(kpiDefinitions.id, kpiId), isNull(kpiDefinitions.deletedAt))).limit(1))[0]; if (!kpi) throw new HttpError(404, "KPI not found", "NOT_FOUND"); await requireModel(kpi.modelId); await requireBusinessContextPermission(user, kpi.dataSourceId, "KPI_VALIDATE"); if (!["DRAFT", "CHANGES_REQUESTED"].includes(kpi.status)) throw new HttpError(409, "Create a new KPI version before validation", "IMMUTABLE_KPI_VERSION");
